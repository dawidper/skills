...
- Use `Read`, `Glob` and `Grep` for inspection and `Bash` for repository commands
  when available. Use `Edit` or `Write` for authorized file changes. Reviewer
  writes remain limited to the handoff and isolated review evidence.
