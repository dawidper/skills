...
- Prefer a persistent Monitor when available; otherwise use the product's interruptible wait/sleep mechanism, then a read-only file check. Prefer roughly 30–60-second intervals; avoid busy polling and individual blocking waits longer than 60 seconds. Never invent a Monitor tool or leave an untracked background shell loop.
