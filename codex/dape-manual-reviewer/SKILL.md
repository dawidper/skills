...
- Use the current session's file and shell tools. Where exposed, use
  ...
  to isolated review evidence and the round archive.
...
- Do not sleep, poll or hold the session open waiting for a submission. The
  round ends when the verdict block is delivered; the operator brings the next
  submission. Do not assume a Claude Code `Monitor` tool exists, and never
  claim a background watch after ending the turn.
